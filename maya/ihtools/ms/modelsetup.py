...
class ArrangeScene():
    ...
    def remove_maya_swatches(self, **kwargs):
        ...
        maya_swatches_path = "{}/.mayaSwatches".format(os.path.split(file_path)[0])

        if os.path.isdir(maya_swatches_path) == True:
    ...
    def deleat_place_2D_textures(self, **kwargs):
        ...
        place_2D_textures = [pt for pt in cmds.ls(st=1) if "place2dTexture" in pt]
...
class ArrangeModels():
    ...
    def move_to_origin(self, **kwargs):
        ...
        for transform in transforms:
            ...
            rot = list(cmds.getAttr("{}.rotate".format(transform))[0])
            scl = list(cmds.getAttr("{}.scale".format(transform))[0])

            for i in range(3):
                pp[i] *= -1
        
                cmds.setAttr("{}.translate".format(transform), trs[0] + pp[0], trs[1] + pp[1], trs[2] + pp[2], type="double3")
                cmds.setAttr("{}.rotate".format(transform), 0, 0, 0, type = "double3")
                cmds.setAttr("{}.scale".format(transform), 1, 1, 1, type = "double3")
    ...
    def rename_SGs(self, **kwargs):
        ...
        SGs = gn.get_SGs(transforms)
        for SG in SGs:
            mat_name = gn.get_material(SG)
            print(mat_name)
    ...
    def mold_uv(self, **kwargs):
        ...
        for peace_selected in select_transforms:
            ...
            for i, uvli in enumerate(select_uv_list):
                if i >= len(uv_list) : break
                if uv_list[i] == uvli :continue
...
class ProjectArrangeModels():
    ...
    def mesh_structure(self, transforms, mesh_name = "Other", group_name = "Mesh"):
        ...
        other = cmds.rename(transforms[0], mesh_name)
    ...
    def rename_material_texture_name(self, **kwargs):
        ...
        gn = getter.GetNode()
        gdp = getter.GetDirPath()

        SGs = gn.get_SGs(transforms)

        for SG in SGs:
            mat_name = gn.get_material(SG)
            texture_path = gdp.get_texture_path(gdp.get_texture_file(mat_name, attr))
            ...
            if (not head or head == "") and (not tail_end or tail_end == ""):
                ...
                new_mat_name = "_".join([split_file_name[name_position]] + [tail_end])  
            elif not tail_end or tail_end == "":
                new_mat_name = "_".join([head] + [split_file_name[name_position]])
            else:
                new_mat_name = "_".join([head] + [split_file_name[name_position]] + [tail_end])
    ...
    def rename_file_image_name(self, **kwargs):
        ...
        for SG in SGs:
            mat_name = gn.get_material(SG)
            file_name = get_dir_path.get_texture_file(mat_name, attr)
